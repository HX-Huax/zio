...
#define PLUGIN_API_VERSION 26 // TeamSpeak 3.1 之后常用的版本号
#endif
...
enum PluginConfigureOffer {
  PLUGIN_OFFERS_NO_CONFIGURE =
      0, /* Plugin does not implement ts3plugin_configure */
  PLUGIN_OFFERS_CONFIGURE_NEW_THREAD, /* Plugin does implement
                                         ts3plugin_configure and requests to run
                                         this function in an own thread */
  PLUGIN_OFFERS_CONFIGURE_QT_THREAD /* Plugin does implement ts3plugin_configure
                                       and requests to run this function in the
                                       Qt GUI thread */
};
